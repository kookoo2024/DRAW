...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    // Serve the app under a sub-path (e.g. https://home.150700.xyz/draw/)
    // so all asset URLs are resolved relative to the deployment prefix.
    base: "/draw/",
    build: {
      outDir: "build",
      rollupOptions: {
    ...
    plugins: [
      ...
      ViteEjsPlugin(),
      VitePWA({
        // Disable PWA for LAN-only deployments to avoid stale service-worker
        // caches masking updates across classroom browsers. Set
        // VITE_APP_DISABLE_PWA=true in .env to turn the SW off entirely.
        disable: envVars.VITE_APP_DISABLE_PWA === "true",
